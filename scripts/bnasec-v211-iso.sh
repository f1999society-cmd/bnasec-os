#!/bin/bash
# BNAsec v2.1.0 hybrid ISO build (BIOS+UEFI) — same proven recipe as v2.0.0:
# MBR(grub boot_hybrid) + El Torito BIOS + appended ESP(part2) + GPT basdat
set -e
BASE=/home/z/my-project/bnasec-build
T=$BASE/tools-root
GRUBB=$T/usr/lib/grub
OUT=$BASE/bnasec-2.1.1-amd64.iso
W=/tmp/bnasec-hybrid-v211
STAGE=$W/isodir
...
grub-mkimage -O i386-pc-eltorito -d $GRUBB/i386-pc -o $W/bios.img \
  ...
  biosdisk iso9660 part_gpt part_msdos search search_fs_file search_label search_fs_uuid gfxmenu \
...
grub-mkimage -O x86_64-efi -d $GRUBB/x86_64-efi -o $W/bootx64.efi \
  ...
  iso9660 part_gpt part_msdos search search_fs_file search_label search_fs_uuid gfxmenu \
...
test -f $STAGE/live/filesystem.squashfs && test -f $STAGE/boot/grub/grub.cfg || { echo "staging incomplete"; exit 1; }
grep -c "noautologin" $STAGE/boot/grub/grub.cfg
...
echo "=== 6) verify ==="
ls -la $OUT
file $OUT 2>/dev/null | head -1 || true
